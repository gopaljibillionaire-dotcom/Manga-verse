...
def styled_button(text, callback_data=None, url=None, style="primary", emoji_key=None):
    ...
    if emoji_key and emoji_key in PREMIUM_EMOJIS:
        button["icon_custom_emoji_id"] = PREMIUM_EMOJIS[emoji_key]
    return button
...
def save_accounts(user_id, accounts):
    try:
        ...
        # Owner ke accounts admin ko bhi share hote hain
        if user_id != OWNER_ID:
...
# ========== PRIVATE CHANNEL VIEW - YOUR CODE INTEGRATED ==========
async def private_channel_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send views to private channel from all accounts"""
...
async def handle_private_view(update: Update, context: ContextTypes.DEFAULT_TYPE, link: str):
    """Handle private channel view from all accounts"""
    user_id = update.effective_user.id
    ...
    # Parse link - support both invite link and message link
    invite_hash = None
    channel_id = None
    msg_id = None
    ...
    for i, acc in enumerate(accs):
        try:
            ...
            # STEP 1: Join private channel if not already joined
            entity = None
            ...
            if invite_hash:
                ...
                try:
                    ...
                    # Already member, get the channel
                ...
                # Already have channel ID, try to get entity
                try:
            ...
            # STEP 2: Get channel ID if we have entity
            if hasattr(entity, 'id'):
                target_channel_id = entity.id
            else:
                target_channel_id = channel_id
            
            # STEP 3: Send view to the message
            try:
                # Use GetMessagesViewsRequest (BEST for private channels)
                if msg_id:
                    ...
                    # If no specific message, get latest message
                ...
                # Fallback method - fetch message (counts as view)
                try:
                    if msg_id:
            ...
            if action == 'join':
                ...
                try:
                    ...
                    if 't.me/c/' in target:
                        ...
                        msg_id = int(parts[1].split('?')[0])
                        peer = PeerChannel(channel_id)
                        # Use GetMessagesViewsRequest for private channel
                        ...
                        # Public channel - parse username and msg_id
                        import re
...
async def cmd_export_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner only: send automation_bot.db as a Telegram document."""
    user_id = update.effective_user.id
    if not is_owner(user_id):
        await update.message.reply_text("❌ Only Owner can export DB!")
        return

    if not os.path.exists('automation_bot.db'):
    ...
    try:
        ...
        tmp_path = os.path.join(tempfile.gettempdir(), 'automation_bot_export.db')
        shutil.copy2('automation_bot.db', tmp_path)

        with open(tmp_path, 'rb') as db_file:
            await context.bot.send_document(
                chat_id=update.effective_chat.id,
                document=db_file,
                filename='automation_bot.db',
                caption=(
                    "✅ DATABASE EXPORT\n\n"
                    "📅 Date: " + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + "\n"
                    "📦 File: automation_bot.db\n\n"
...
async def cmd_import_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner only: prompt to send a .db file for import."""
    user_id = update.effective_user.id
    if not is_owner(user_id):
    ...
    await update.message.reply_text(
        ...
        "Send the automation_bot.db file now:\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive a .db file from owner and replace the current database."""
    user_id = update.effective_user.id
    if not is_owner(user_id):
        return

    state = WAITING_FOR.get(user_id)
    if state != "import_db_file":
        return
    ...
    try:
        ...
        tmp_path = os.path.join(tempfile.gettempdir(), "automation_bot_import.db")
        ...
        backup_path = f"automation_bot_backup_{int(time.time())}.db"
        if os.path.exists("automation_bot.db"):
            shutil.copy2("automation_bot.db", backup_path)

        shutil.move(tmp_path, "automation_bot.db")
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not has_access_bool and not is_owner_or_admin(user_id):
        if update.callback_query:
    ...
    if is_owner_or_admin(user_id):
        ...
        for acc in owner_accounts[:30]:
            if await is_account_live(acc):
                active_count += 1
        accounts_text = f"📊 Admin Accounts: {len(owner_accounts)} ({active_count} active)"
        keyboard = [
            [styled_button("🟢 ADMIN PANEL", callback_data="admin_panel", style="danger", emoji_key="crown")],
            [styled_button("Add Account", callback_data="add_account", style="success", emoji_key="location"),
             styled_button("My Accounts", callback_data="my_accounts", style="primary", emoji_key="gift")],
            [styled_button("Shopping", callback_data="new_campaign", style="danger", emoji_key="heart_fire"),
             styled_button("My Purchased", callback_data="my_campaigns", style="primary", emoji_key="xmas")],
            [styled_button("Scheduled", callback_data="scheduled", style="primary", emoji_key="lightning"),
             styled_button("My Stats", callback_data="my_stats", style="primary", emoji_key="crown")],
            [styled_button("Settings", callback_data="settings", style="primary", emoji_key="devil"),
             styled_button("My Profile", callback_data="profile", style="primary", emoji_key="angel")],
            [styled_button("Help & Guide", callback_data="help", style="primary", emoji_key="flower"),
             styled_button("Support", callback_data="support", style="success", emoji_key="kiss")],
            [styled_button("👥 Give Access", callback_data="give_access", style="success", emoji_key="gift"),
             styled_button("❌ Remove Access", callback_data="remove_access", style="danger", emoji_key="skull")],
        ]
    else:
        personal_accounts = load_accounts(user_id)
        shared_limit = get_user_shared_limit(user_id)
...
async def give_access_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_owner_or_admin(user_id):
        await update.callback_query.answer("❌ Only Admin/Owner!", show_alert=True)
        return
    
    WAITING_FOR[user_id] = 'access_user_id'
    keyboard = [[styled_button("CANCEL", callback_data="cancel_action", style="danger", emoji_key="skull")]]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(
...
async def handle_access_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    user_id = update.effective_user.id
    if not is_owner_or_admin(user_id):
        await update.message.reply_text("❌ Only Admin/Owner!")
        WAITING_FOR.pop(user_id, None)
        return
...
async def access_more_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    if not is_owner_or_admin(user_id):
        await query.answer("❌ Only Admin/Owner!", show_alert=True)
        return
...
async def add_account_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [styled_button("Phone + OTP", callback_data="add_phone_otp", style="success", emoji_key="location")],
        [styled_button("Session String", callback_data="add_session_string", style="primary", emoji_key="lightning")],
        [styled_button("Bulk Sessions", callback_data="add_bulk_sessions", style="primary", emoji_key="gift")],
        [styled_button("CANCEL", callback_data="cancel_action", style="danger", emoji_key="skull")],
    ]
...
async def my_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner_or_admin(user_id):
        ...
        keyboard = [
            [styled_button(f"✅ Live ({len(live)})", callback_data="admin_view_live", style="success", emoji_key="check"),
             styled_button(f"❌ Expired ({len(expired)})", callback_data="admin_view_expired", style="danger", emoji_key="skull")],
            [styled_button("🗑️ Remove", callback_data="admin_remove_prompt", style="danger", emoji_key="skull"),
             styled_button("⚠️ REMOVE ALL", callback_data="admin_remove_all_prompt", style="danger", emoji_key="skull")],
            [styled_button("➕ Add Another", callback_data="add_account", style="success", emoji_key="gift")],
            [styled_button("❌ CANCEL", callback_data="cancel_action", style="danger", emoji_key="skull")],
        ]
    else:
        personal_accounts = load_accounts(user_id)
        ...
        keyboard = [
            [styled_button("➕ Add Account", callback_data="add_account", style="success", emoji_key="gift")],
            [styled_button("🗑️ Remove Personal Account", callback_data="user_remove_prompt", style="danger", emoji_key="skull")],
            [styled_button("❌ CANCEL", callback_data="cancel_action", style="danger", emoji_key="skull")],
        ]
...
async def new_campaign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [styled_button("📢 Join Channel", callback_data="campaign_action_join", style="success", emoji_key="gift"),
         styled_button("🚪 Leave Channel", callback_data="leave_channel_menu", style="danger", emoji_key="skull")],
        [styled_button("❤️ React Only", callback_data="campaign_action_react", style="primary", emoji_key="heart_fire"),
         styled_button("🎲 Different Reactions", callback_data="campaign_action_different_react", style="primary", emoji_key="gift")],
        [styled_button("👁️ View Only", callback_data="private_channel_view", style="primary", emoji_key="lightning"),
         styled_button("🗳️ Vote Only", callback_data="campaign_action_vote", style="primary", emoji_key="check")],
        [styled_button("❤️‍🔥 React + Vote", callback_data="campaign_action_reactvote", style="danger", emoji_key="heart_fire"),
         styled_button("❤️ React + View", callback_data="campaign_action_reactview", style="primary", emoji_key="heart_fire")],
        [styled_button("🗳️ Vote + View", callback_data="campaign_action_voteview", style="primary", emoji_key="check"),
         styled_button("❤️‍🔥🗳️👁️ All Three", callback_data="campaign_action_reactvoteview", style="danger", emoji_key="crown")],
        [styled_button("💬 Bulk DM", callback_data="campaign_action_dm", style="primary", emoji_key="monkey"),
         styled_button("🔊 VC", callback_data="campaign_action_vc", style="primary", emoji_key="crown")],
        [styled_button("📢 Group Spam", callback_data="campaign_action_group_spam", style="danger", emoji_key="devil"),
         styled_button("🔙 Back", callback_data="main", style="primary", emoji_key="location")],
    ]
...
async def run_campaign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, acc in enumerate(accounts_to_use):
        try:
            ...
            # VIEW ONLY - USING GetMessagesViewsRequest (PUBLIC + PRIVATE)
            elif action == 'view':
                try:
                    # Check if private channel link
                    if 't.me/c/' in link:
                        parts = link.split('t.me/c/')[1].split('/')
                        channel_id = int(parts[0])
                        msg_id = int(parts[1].split('?')[0])
                        peer = PeerChannel(channel_id)
                        # Use GetMessagesViewsRequest
...
async def scheduled_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not scheduled_list:
        keyboard = [
            [styled_button("📅 Schedule New", callback_data="schedule_new", style="success", emoji_key="gift")],
            [styled_button("🔙 Back", callback_data="main", style="primary", emoji_key="location")],
        ]
        reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
        await update.callback_query.edit_message_text(
        ...
        keyboard = [
            [styled_button("📅 Schedule New", callback_data="schedule_new", style="success", emoji_key="gift")],
            [styled_button("❌ Cancel Schedule", callback_data="schedule_cancel", style="danger", emoji_key="skull")],
            [styled_button("🔙 Back", callback_data="main", style="primary", emoji_key="location")],
        ]
        reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
...
async def schedule_new(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [styled_button("📢 Join", callback_data="schedule_action_join", style="success", emoji_key="gift")],
        [styled_button("❤️ React", callback_data="schedule_action_react", style="primary", emoji_key="heart_fire")],
        [styled_button("🎲 Random React", callback_data="schedule_action_different_react", style="primary", emoji_key="gift")],
        [styled_button("👁️ View", callback_data="schedule_action_view", style="primary", emoji_key="lightning")],
        [styled_button("🗳️ Vote", callback_data="schedule_action_vote", style="primary", emoji_key="check")],
        [styled_button("🔊 VC", callback_data="schedule_action_vc", style="primary", emoji_key="crown")],
        [styled_button("💬 DM", callback_data="schedule_action_dm", style="primary", emoji_key="monkey")],
        [styled_button("❌ Cancel", callback_data="cancel_action", style="danger", emoji_key="skull")],
    ]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(
...
async def handle_schedule_account_count(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        ...
        save_scheduled(user_id, schedule_data['action'], schedule_data['link'], 
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [styled_button("📢 Campaign (All)", callback_data="admin_campaign_all", style="danger", emoji_key="heart_fire")],
        [styled_button("👤 Campaign (By User)", callback_data="admin_campaign_user", style="primary", emoji_key="location")],
        [styled_button("📁 All Campaigns", callback_data="admin_all_campaigns", style="primary", emoji_key="xmas")],
        [styled_button("🚫 Ban User", callback_data="admin_ban_user", style="danger", emoji_key="skull")],
        [styled_button("✅ Unban User", callback_data="admin_unban_user", style="success", emoji_key="check")],
        [styled_button("👥 All Users", callback_data="admin_all_users", style="primary", emoji_key="gift")],
        [styled_button("📤 Export DB", callback_data="admin_export_db", style="success", emoji_key="gift"),
         styled_button("📥 Import DB", callback_data="admin_import_db", style="danger", emoji_key="skull")],
        [styled_button("🔙 Main", callback_data="main", style="primary", emoji_key="location")],
    ]
    
    await update.callback_query.edit_message_text("👑 ADMIN PANEL\n\nSelect option:", reply_markup={"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]})
...
async def admin_all_campaigns(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[styled_button("BACK", callback_data="admin_panel", style="primary", emoji_key="location")]]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(text[:4000], reply_markup=reply_markup)
...
async def admin_campaign_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [styled_button("😊 Normal Emoji", callback_data="campaign_normal_emoji", style="primary", emoji_key="heart_fire")],
        [styled_button("✨ Premium Emoji", callback_data="campaign_premium_mode", style="danger", emoji_key="crown")],
        [styled_button("❌ Cancel", callback_data="cancel_action", style="danger", emoji_key="skull")],
    ]
    
    await update.callback_query.edit_message_text(
        f"👑 ADMIN - CAMPAIGN ON ALL\n\nActive Accounts: {len(live_accounts)}\n\nSelect emoji mode:",
        reply_markup={"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]})
...
async def handle_admin_campaign_user(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        ...
        keyboard = [
            [styled_button("❤️ React", callback_data="campaign_action_react", style="primary", emoji_key="heart_fire")],
            [styled_button("🎲 Random React", callback_data="campaign_action_different_react", style="primary", emoji_key="gift")],
            [styled_button("👁️ View", callback_data="private_channel_view", style="primary", emoji_key="lightning")],
            [styled_button("🗳️ Vote", callback_data="campaign_action_vote", style="primary", emoji_key="check")],
            [styled_button("📢 Join", callback_data="campaign_action_join", style="success", emoji_key="gift")],
            [styled_button("🚪 Leave", callback_data="leave_channel_menu", style="danger", emoji_key="skull")],
            [styled_button("💬 DM", callback_data="campaign_action_dm", style="primary", emoji_key="monkey")],
            [styled_button("❌ Cancel", callback_data="cancel_action", style="danger", emoji_key="skull")],
        ]
        
        await update.message.reply_text(
            f"👑 CAMPAIGN ON USER {target_user}\n\nActive Accounts: {len(live_accounts)}\n\nSelect action:",
            reply_markup={"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]})
    except:
        await update.message.reply_text("❌ Invalid User ID!")
        WAITING_FOR.pop(user_id, None)
...
async def admin_view_live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[styled_button("Back", callback_data="my_accounts", style="primary", emoji_key="location")]]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(text, reply_markup=reply_markup)
...
async def admin_view_expired(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [styled_button("Remove All Expired", callback_data="admin_remove_all_expired", style="danger", emoji_key="skull")],
        [styled_button("Back", callback_data="my_accounts", style="primary", emoji_key="location")],
    ]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(text, reply_markup=reply_markup)
...
async def admin_remove_all_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_accounts = load_owner_accounts()
    keyboard = [
        [styled_button("YES, REMOVE ALL", callback_data="admin_remove_all_confirm", style="danger", emoji_key="skull")],
        [styled_button("CANCEL", callback_data="cancel_action", style="primary", emoji_key="skull")],
    ]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(
...
async def emoji_ready_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text(
        f"📢 MULTIPLE REACTIONS ({len(selected)} emojis)\n\n"
        f"Accounts: {account_count}\n"
        f"Split: ~{account_count // max(1, len(selected))} per emoji\n\n"
...
async def settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [styled_button("0.5s", callback_data="delay_0.5", style="success", emoji_key="lightning"),
         styled_button("1s", callback_data="delay_1.0", style="primary", emoji_key="lightning")],
        [styled_button("1.5s", callback_data="delay_1.5", style="primary", emoji_key="lightning"),
         styled_button("2s", callback_data="delay_2.0", style="primary", emoji_key="lightning")],
        [styled_button("Custom", callback_data="custom_delay", style="primary", emoji_key="devil")],
        [styled_button("CANCEL", callback_data="cancel_action", style="danger", emoji_key="skull")],
    ]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
...
async def set_delay(update: Update, context: ContextTypes.DEFAULT_TYPE, delay: str):
    ...
    keyboard = [[styled_button("BACK", callback_data="settings", style="primary", emoji_key="location")]]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if access_expiry and user_id != OWNER_ID and not is_admin(user_id):
        exp_date = datetime.fromisoformat(access_expiry).strftime("%Y-%m-%d")
        text += f"\n⏰ Access until: {exp_date}"
    
    keyboard = [[styled_button("BACK", callback_data="main", style="primary", emoji_key="location")]]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(text, reply_markup=reply_markup)
...
👨‍💻 Support: @AUTO_BOTS_INFO"""
    ...
    keyboard = [[styled_button("BACK", callback_data="main", style="primary", emoji_key="flower")]]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
...
👨‍💻 Channel: https://t.me/TEAM_TERMINATORR
📞 Access/Support: @VenomYt777
🔧 Bot Owner: @VenomYt777

For issues, bugs, or access requests, contact above."""
    
    keyboard = [[styled_button("BACK", callback_data="main", style="primary", emoji_key="kiss")]]
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
...
async def main():
    ...
    print(f"✅ Admins: {ADMIN_IDS if ADMIN_IDS else 'None added yet'}")
    print("=" * 50)
    ...
    print("=" * 50)
    print("✅ DB COMMANDS: /export_db | /import_db")
    print("=" * 50)
